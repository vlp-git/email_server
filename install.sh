...
F_mail_postfix() {
	...
	postconf -e "smtp_tls_protocols = TLSv1.3, TLSv1.2, !TLSv1.1, !TLSv1, !SSLv2, !SSLv3"#Les protocoles que l'on accepte d'utiliser sont uniquement 1.2 et 1.3
	...
	postconf -e "smtp_tls_mandatory_exclude_ciphers = MD5, DES, ADH, RC4, PSD, SRP, 3DES, eNULL, aNULL"#Dans les négociation on refuse explicitement c'est protocoles immonde
	postconf -e "smtp_tls_exclude_ciphers = MD5, DES, ADH, RC4, PSD, SRP, 3DES, eNULL, aNULL"# On les exclue également de l'utilisation
	postconf -e "smtp_tls_mandatory_protocols = TLSv1.3, TLSv1.2, !SSLv2, !SSLv3, !TLSv1, !TLSv1.1"#On négocie uniquement 1.2 et 1.3 (pas obligatoire mais ça mange pas de pain)
	#### SMTPD global
	...
	postconf -e "smtpd_tls_mandatory_exclude_ciphers = MD5, DES, ADH, RC4, PSD, SRP, 3DES, eNULL, aNULL"#Dans les négociation on refuse explicitement c'est protocoles immonde
	postconf -e "smtpd_tls_exclude_ciphers = MD5, DES, ADH, RC4, PSD, SRP, 3DES, eNULL, aNULL"# On les exclue également de l'utilisation
	postconf -e "smtpd_tls_mandatory_protocols = TLSv1.3, TLSv1.2, !SSLv2, !SSLv3, !TLSv1, !TLSv1.1"#On négocie uniquement 1.2 et 1.3 (pas obligatoire mais ça mange pas de pain)
	postconf -e "smtpd_tls_received_header = yes"
	#### Autre config de sécurité
	postconf -e "tls_preempt_cipherlist = yes"#Choix final du protocol par le serveur(client par défaut)/ peu poser problème si le client n'est pas conciliant, mais normalement peu de chance de se retrouver en face d'une impasse. Choix qui peut faire polémique
	...
	postconf -e "tls_high_cipherlist = ECDHE-ECDSA-CHACHA20-POLY1305:ECDHE-ECDSA-AES256-GCM-SHA384:ECDHE-RSA-CHACHA20-POLY1305:ECDHE-RSA-AES256-GCM-SHA384:!aNULL:!eNULL:!LOW:!3DES:!MD5:!EXP:!PSK:!DSS:!RC4:!SEED:!SHA1"#J'ai gardé la configuration précédente avec l'interdiction absolue des protocoles faible/null
